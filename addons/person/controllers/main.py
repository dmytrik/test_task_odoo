...
class PersonController(http.Controller):
    ...
    def persons_page(self, **kwargs):
        ...
        html_file_path = os.path.join(
            module_path, "static", "src", "html", "people.html"
        )
    ...
    @http.route(
        "/persons/data/", type="http", auth="public", website=True, methods=["GET"]
    )
    ...
    def create_person_form(self, **kwargs):
        module_path = os.path.realpath(os.path.join(os.path.dirname(__file__), ".."))
        html_file_path = os.path.join(
            module_path, "static", "src", "html", "create_person.html"
        )
    ...
    @http.route(
        "/api/companies/", type="http", auth="public", methods=["GET"], csrf=False
    )
    def get_companies(self, **kwargs):
        try:

            companies = request.env["res.company"].search_read(
                domain=[], fields=["id", "name"], limit=100
            )

            response_data = {"result": companies}
            ...
            error_response = {
                "error": {
                    "code": 500,
                    "message": str(e),
                    "data": {"name": "Exception", "debug": str(e)},
                }
            }
            return http.Response(
